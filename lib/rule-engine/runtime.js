...
const globToRegexSource = (pattern, mode) => {
  const globMode = mode === "path" || mode === "host" ? mode : "text";
  const unit = globMode === "path" ? "[^/]*" : globMode === "host" ? "[^.]*" : ".*";
  let out = "";
...
const compileGlobRegex = (pattern, caseMode, globMode = "text") => {
  ...
  let source = globToRegexSource(pattern, mode);
  if (mode === "path" && pattern.endsWith("/**")) {
    const withoutTrailingGlobstar = globToRegexSource(pattern.slice(0, -3), mode);
    source = `(?:${source}|${withoutTrailingGlobstar})`;
  }
  const regex = new RegExp(`^${source}$`, caseMode === "sensitive" ? "" : "i");
